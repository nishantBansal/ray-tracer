...
public:
	...
	bool IsPlane (void) { return (type == SceneObjectType::Plane); }
...
/*
	ScenePlane Class - The plane scene object

	Plane object derived from the SceneObject
*/
...
public:
	...
	// -- Constructors & Destructors --
	ScenePlane (void) : SceneObject ("Plane", SceneObjectType::Plane) {}
	ScenePlane (std::string nm) : SceneObject (nm, SceneObjectType::Plane) {}
};
/*
...
public:
	...
	~Scene (void)
	{
		...
		for (unsigned int n = 0; n < numObj; n++)
		{
			...
			else if (sceneObj->IsPlane ())
			{
				delete ((ScenePlane *)sceneObj);
			}
	...
	// - GetDepth - Return the Depth String
	const int GetDepth (void) { return m_Depth; }
...
bool Scene::Load (char *filename)
{
	...
	char *temp = CHECK_ATTR(sceneXML.getAttribute("depth"));
	m_Depth = atoi(temp);
	if(m_Depth <= 0)
		m_Depth = 1;
	...
	if (!tempNode.isEmpty ())
	{
		...
		for (unsigned int n = 0; n < numLights; n++)
		{
			...
			tempLight.ambient = ParseColor (tempLightNode.getChildNode("ambient"));
			tempLight.diffuse = ParseColor (tempLightNode.getChildNode("diffuse"));
			tempLight.specular = ParseColor (tempLightNode.getChildNode("specular"));
	...
	if (!tempNode.isEmpty ())
	{
		...
		for (unsigned int n = 0; n < numMaterials; n++)
		{
			...
			tempMaterial.ambient = ParseColor (tempMaterialNode.getChildNode("ambient"));
			...
			tempMaterial.refraction = ParseColor (tempMaterialNode.getChildNode("refraction"));
			tempMaterial.refraction_index = atof(CHECK_ATTR(tempMaterialNode.getChildNode("refraction").getAttribute("refraction_index")));
	...
	if (!tempNode.isEmpty ())
	{
		...
		for (unsigned int n = 0; n < numObjects; n++)
		{
			...
			else if (!strcasecmp(tempObjectNode.getName (), "plane"))
			{
				// Load a Sphere
				ScenePlane *tempPlane = new ScenePlane ();
				tempPlane->name = CHECK_ATTR(tempObjectNode.getAttribute("name"));
				tempPlane->material = CHECK_ATTR(tempObjectNode.getAttribute("material"));
				tempPlane->texture = CHECK_ATTR(tempObjectNode.getAttribute("texture"));
				tempPlane->scale = ParseXYZ (tempObjectNode.getChildNode("scale"));
				tempPlane->rotation = ParseXYZ (tempObjectNode.getChildNode("rotation"));
				tempPlane->position = ParseXYZ (tempObjectNode.getChildNode("position"));
				tempPlane->P[0] = ParseXYZ (tempObjectNode.getChildNode("p0"));
				tempPlane->P[1] = ParseXYZ (tempObjectNode.getChildNode("p1"));
				tempPlane->P[2] = ParseXYZ (tempObjectNode.getChildNode("p2"));
				tempPlane->P[3] = ParseXYZ (tempObjectNode.getChildNode("p3"));
				m_ObjectList.push_back (tempPlane);
			}
